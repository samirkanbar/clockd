        ...
        <tbody>
          {weekDates.map((date, i) => {
            ...
            return (
              <tr
                key={date}
                className={`border-b border-zinc-800/50 last:border-0 ${isToday ? 'bg-amber-400/5' : ''}`}
              >
                <td className="px-4 py-2.5 font-mono text-sm">
                  <span className={isToday ? 'text-amber-400' : 'text-zinc-400'}>
                ...
                <td className="px-4 py-2.5 font-mono text-sm text-right">
                  <span className={secs > 0 ? (isToday ? 'text-amber-400' : 'text-zinc-300') : 'text-zinc-700'}>
                    {formatDuration(secs)}
                  </span>
                </td>
              </tr>
